...
| Doing  | Public MVP needs Vercel and Grok delivery plan.                     | Public deployment, Grok transport, server action, and access gate exist; MVP demo acceptance requires one post-merge production smoke with `live-ai`, `xai`, `cold-open-lab.v2`, status `success`, and at least three accepted variants. |
...
| Doing  | Real deployed Grok smoke is not yet proven.                         | After merge, rerun production smoke; accept only `live-ai`/`xai`/`cold-open-lab.v2` success with at least three variants, otherwise keep MVP demo blocked and iterate the prompt.                                                        |
